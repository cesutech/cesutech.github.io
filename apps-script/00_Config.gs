...
var CONFIG_PADRAO = [
  ...
  { chave: 'coordenadores_gerais', valor: '', descricao: 'Quem é COORDENADOR GERAL, separado por vírgula. Todo e-mail de admin_emails que NÃO estiver aqui é PROFESSOR: vê e exporta as listas de inscritos e disciplinas, e não remove projeto, não anula, não promove, não importa lista oficial e não configura. CAMPO VAZIO = todos os e-mails de admin_emails são coordenadores gerais (é o estado de origem do sistema, e é por isso que implantar sem tocar no banco não muda nada para ninguém). Um e-mail aqui que não esteja em admin_emails não vale nada: quem ENTRA é decidido só por admin_emails, e o nível nunca é porta. O caminho certo para mexer nisto é Configurações > Quem tem acesso ao painel, que derruba a sessão de quem mudou de nível — este campo existe porque a tela de Configurações lista toda chave que exista no banco, e uma chave que ia aparecer de qualquer jeito é melhor que apareça explicada.' },
  ...
  { chave: 'inscricoes_inicio', valor: '', descricao: 'Quando o PERÍODO GERAL de inscrição começa, no formato AAAA-MM-DD HH:MM (ex.: 2026-10-01 08:00), na hora de Brasília — a mesma de "Recebido em". Antes desse instante o site mostra "as inscrições abrem em..." com o tempo que falta, o botão de inscrever fica desligado e o servidor recusa qualquer envio dizendo a data. VAZIO = sem data de início (já está aberto, como hoje). Vale para TODOS os projetos de uma vez: o "Inscrições abertas" de cada projeto continua fechando o projeto sozinho, e cadastro_aberto=NAO fecha tudo de qualquer jeito. Valor fora do formato é IGNORADO e anotado no log — uma data digitada torta não pode fechar a inscrição por acidente.' },
  { chave: 'inscricoes_fim', valor: '', descricao: 'Quando o PERÍODO GERAL de inscrição termina, no mesmo formato de inscricoes_inicio (ex.: 2026-10-30 18:00). Até lá o site mostra "abertas até..." com o tempo que falta; a partir desse minuto a lista deixa de mostrar vagas restantes, o selo vira "Encerradas", o botão some e o servidor recusa o envio dizendo a data — inclusive de quem já estava com o formulário aberto. VAZIO = sem data de fim (nunca fecha sozinho, como hoje). As DUAS chaves vazias = o site de sempre, sem faixa nem contador. Valor fora do formato é IGNORADO e anotado no log. O site lê isto com até 30 s de atraso (cache da rota).' },
  ...
  { chave: 'matricula_digitos', valor: '7', descricao: 'Quantos dígitos a matrícula tem — 7, medido nas listas da secretaria. O formulário RECUSA matrícula com outro tamanho, dizendo ao aluno o tamanho certo: é o que impede a matrícula digitada errada de entrar e quebrar o cruzamento com a lista oficial. Zero à esquerda é aceito e removido (a secretaria escreve 09110001, o aluno lê 9110001 no portal, e as duas são a mesma matrícula). Vazio, texto ou zero NÃO desligam a régua: o sistema valida com 7 e anota o problema no log — uma chave digitada errado não pode abrir a porta para matrícula de qualquer tamanho.' },
  { chave: 'exigir_cpf', valor: 'NAO', descricao: 'SIM/NAO — o formulário do CESUTECH não pede CPF; a chave é a matrícula.' },
  { chave: 'aluno_projeto_unico', valor: 'NAO', descricao: 'SIM = um projeto ATIVO por matrícula, nas DUAS portas. No formulário do aluno: quem já está em outro projeto não é recusado — é avisado e pode TROCAR, e a troca cancela a inscrição anterior (ela vai para as anuladas do Geral, com o protocolo novo no lugar do antigo). Três condições para a troca: o e-mail do envio tem de ser o mesmo da inscrição anterior, a inscrição anterior não pode ter sido feita pela coordenação (essa é migrada por Alunos → Editar → Projeto) e o projeto novo tem de ter vaga na hora de confirmar — sem vaga, nada muda e a anterior continua valendo. No painel, em Incluir aluno: incluir quem já está em outro projeto passa a PERGUNTAR antes de gravar, em vez de gravar e avisar depois (a coordenação confirma e inclui do mesmo jeito — a porta dela nunca é barrada). NAO (padrão) = as duas portas do jeito de sempre: o formulário não recusa ninguém, o painel inclui sem perguntar, e os dois apenas sinalizam o outro projeto. A regra vale só por MATRÍCULA: sem ela (exigir_matricula=NAO), nem pergunta nem troca. Antes de virar para SIM, leia o checklist no README — a base de hoje tem duplicidades, e a regra é só para frente.' },
